...
export default function PokerConsole(){
  ...
  const rangeCellStyle=(player:HandState["players"][number],label:string)=>{
    const cell={...EMPTY_RANGE_CELL,...player.stats.preflopRange[label]};
      ...
      <div className="layout-panel">
        <Tabs value={activeTab} onValueChange={value=>{setActiveTab(value);setShowStats(value==="stats");setShowRange(value==="range");window.localStorage.setItem(DISPLAY_TAB_KEY,value);}} className="layout-tabs">
          ...
          <TabsContent value="range" className="range-tab">
            <div className="layout-heading"><div><span className="eyebrow">HAND RANGE HEATMAP</span><h1>All Player Ranges</h1><p>全シートを同時表示します。明度が参加率、色相がCall／Raise傾向です。各セルの詳細はマウスオーバーで確認できます。</p></div></div>
            <div className="layout-editor" ref={activeTab==="range"?stageRef:undefined}>
              <div className="editor-canvas" style={{transform:`scale(${editorScale})`}}>
                <div className="capture-area" style={{left:`${layout.area.x}%`,top:`${layout.area.y}%`,width:`${layout.area.width}%`,height:`${layout.area.height}%`}}><span>OBS CAPTURE AREA</span></div>
